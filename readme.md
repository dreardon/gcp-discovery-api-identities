...
3.  **Azure Active Directory (Entra ID) Tenant and Application Registration:**
    ...
    *   An Application Registration in Azure AD with:
        ...
        *   A Workforce Configured Application for SAML
...
4.  **Google Cloud Platform (GCP) Project:**
    ...
    *   **Workforce Identity Federation Configured:**
        ...
        *   An SAML provider within that pool configured to trust your Azure AD application. The provider's audience string will be `GOOGLE_WIF_AUDIENCE`.
...
gcloud run deploy agentspace-api-identities-saml \
  --source . \
  --platform managed \
  --region $REGION \
  ...
  --set-env-vars="AZURE_SAML_IDP_CERT=$AZURE_SAML_IDP_CERT"
  ```